...
const handleCard = async (
  ...
  await page.screenshot({
    path: `./screenshots/${card.issuer}-${card.name}.png`,
    fullPage: true,
  });
  const imageBase64 = fs.readFileSync(
    `./screenshots/${card.issuer}-${card.name}.png`,
    "base64"
  );

  const promptForThisCard = prompt.replaceAll(
    "%CARD_INFO%",
  ...
  if (
    output.includes("No Bonus Visible") ||
    output.includes("Incorrect Card")
  ) {
    console.info(
      chalk.red(`${card.issuer} ${card.name} (${card.url}):`, output)
    );
  } else if (output.includes("Accurate")) {
    console.info(
      chalk.green(`${card.issuer} ${card.name} (${card.url}):`, output)
    );
  } else if (output.includes("Requires Update")) {
    console.info(
      chalk.yellow(`${card.issuer} ${card.name} (${card.url}):`, output)
    );
  } else {
